...
public class DirectionService {
    ...
    public Single<List<DirectionDTO>> getDirections(MapProvider provider, LatLngPair latLngPair) {
        ...
        List<DirectionDTO> directions = directionRepository.findByStartAndEndAndProviderOrderByPositionAsc(latLngPair.getSrc().getPlace(), latLngPair.getDest().getPlace(), provider).stream().map(direction -> new DirectionDTO(direction.getPosition(), direction.getText(), direction.getDistanceInMiles(), direction.getTimeInMinutes())).collect(Collectors.toList());
